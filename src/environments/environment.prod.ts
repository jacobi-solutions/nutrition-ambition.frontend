export const environment = {
  production: true,
  backendApiUrl: 'https://api.nutritionambition.app',
  frontendUrl: 'https://nutritionambition.app',
  storageBucketBaseUrl: 'https://storage.googleapis.com/nutritionambition.app',
  authDebug: false,
  firebase: {
    apiKey: "AIzaSyAUPErnsxJ7DReXiat7PdppoV-Apbpyxt8",
    authDomain: "nutrition-ambition.firebaseapp.com",
    projectId: "nutrition-ambition",
    storageBucket: "nutrition-ambition.firebasestorage.app",
    messagingSenderId: "496562523884",
    appId: "1:496562523884:web:d709330c10dbcc761f1fc9",
    measurementId: "G-9EEG64HG46"
  },
  tenantId: null,
  stripePrices: {
    publicMonthly: 'price_1SXjfMGxHs6evYLlyNrShYnG',
    public6Month: 'price_1SXjhzGxHs6evYLl4JQg9JYf',
    public12Month: 'price_1SXjjVGxHs6evYLlAIejs4vg',
    earlyMonthly: 'price_1SXjoGGxHs6evYLlAErCHX8K',
    early6Month: 'price_1SXjrZGxHs6evYLloB1Tp84m',
    early12Month: 'price_1SXjsKGxHs6evYLlVywxuFHN',
    earlyLifetime: 'price_1SXjstGxHs6evYLltmbRkeod'
  }
};
